...
contract('NOIA Token', async accounts => {
    ...
    function signAuthorization(signer, operator, nonce, authorize) {
        ...
        return web3.eth.sign(signer, msg);
    }
    ...
    it('should fail to transfer while minting', async () => {
        token.transfer(user1, OneToken).should.be.rejected;
    });

    it('should fail to switch on burning before finished minting', async () => {
        token.enableBurn(true).should.be.rejected;
    });
    ...
    it('should fail to finish minting again', async () => {
        token.finishMinting().should.be.rejected;
    });
    ...
    it('should fail to burn before burning is enabled', async () => {
        token.burn(OneToken.mul(0.5), ', ', { from: user1 }).should.be.rejected;
    });
    ...
    it('should be possible to authorize operator with signature', async () => {
        ...
        const nonce = 0;
        const signature = signAuthorization(signer, operator, nonce, AUTHORIZE_OPERATOR);
    ...
    it('should be possible to revoke operator with signature', async () => {
        ...
        const operator = user2;
        const nonce = 1;
        const signature = signAuthorization(signer, operator, nonce, REVOKE_OPERATOR);
    ...
    it('should not be possible to revoke operator with authorize signature', async () => {
        ...
        await token.revokeOperatorWithSignature(operator, revokeNonce, signAuthorization(signer, operator, revokeNonce, AUTHORIZE_OPERATOR), {
            from: user2
        }).should.be.rejected;
    });
    ...
    it('should not be possible to authorize operator with wrong nonce', async () => {
        ...
        const operator = user2;
        const nonce = 999;
        await token.authorizeOperatorWithSignature(operator, nonce, signAuthorization(signer, operator, nonce, AUTHORIZE_OPERATOR), {
            from: user2
        }).should.be.rejected;
    });
